...
let entrypoint_term = Term.(const Core.entrypoint $ recursive $ ignore_files $ ignore_patterns $ from_words $ to_words $ debug_mode)
...
let info =
  ...
  let man = [
    ...
    `P "." ]
  in
  Cmd.info "pattern-renamer" ~version:"v0.0.3" ~doc ~exits:Cmd.Exit.defaults ~man
